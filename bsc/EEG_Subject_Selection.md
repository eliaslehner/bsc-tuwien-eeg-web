...
Add `--plot` to regenerate the benchmark figure + CSV below:

![Subject selection benchmark](figures/subject_selection_benchmark.png)

*Figure generated by `python -m backend.eeg.select_subject --plot` over all 9
subjects (`bsc/figures/subject_selection_benchmark.{png,csv}`). A03 (green) is the
cleanest exemplar.*
